...
gene_ids <- row.names(curr_sim)
generic_genes <- universe[match(row.names(curr_sim), my_universe)]
...
annotation_row_logical <- data.frame(KEGG_INOSITOL_PHOSPHATE_METABOLISM = validation_dt[, 3]) %>%  # :5]) %>%
...
ann_colors <- list(Var1) %>% set_names(abbreviated_names)
