...
if(Sys.info()['sysname'] == "Windows"){
  source("C:/Users/steph/Desktop/Bioinformatics/GEN80436 - Thesis (Cam)/mdipp-1.0.1/scripts/analysis.R")
} else {
...
probes_present_med_dt <- fread("Data/med_meta_data/probes_present_per_dataset.csv")
probes_present_small_dt <- fread("Data/small_meta_data/probes_present_per_dataset.csv")


# Read in the file relating the probe IDs to  the related gene
probe_key <- fread("Data/full_probe_key.csv")

# Read in the MDI output file
# file_path <- "Analysis/MDI_runs/vsn_many_seeds/"
file_path <- "C:/Users/steph/Desktop/MDI runs/"
mdi_output_dir <- c("med_output", "small_output")
gene_sets <- c("med", "small")
num_sets <- length(mdi_output_dir)

rel_file_path <- paste0(file_path, mdi_output_dir)

mdi_output_files <- list()
for(i in 1:num_sets){
  
  d <- rel_file_path[[i]]
  set <- gene_sets[[i]]
  
  mdi_output_files[[set]] <- list.files(path = d, full.names = T, include.dirs = F) %>% 
    grep("csv", ., value = TRUE)
}
num_files <- mdi_output_files %>% lapply(length)
file_names <- mdi_output_files %>% 
...
if (do_heatplot_clusterings) {
  ...
  for (j in 1:num_files) {
    ...
    for (i in 1:num_datasets) {
      ...
      if (save_plots) {
        ph <- pheatmap(compare_tibble$MDI_allocation[j][[i]],
                       main = title,
                       cluster_rows = F,
                       filename = file_name
        )
