...
# Makes labels based upon UMAP coordinates
...
# make dataset for a ggplot of UMAP representation given labels
...
# Using the UMAP plot data, create a ggplot2 object colour by labelling
...
# Plot summayr statistics of the rows of a given dataset and their marginal histograms
...
As previosuly we create a set of labels based upon the UMAP representation of the data to see how points move after the transform.
...
```{r meth_summary_statistics_post}
...
Visualise the relaitonship between the standard deviation and the mean of each gene:

```{r miRNA_summary_statistics_pre}
# Plot summary statistics
...
```{r miRNA_summary_statistics_post}
...
```{r protein_summary_statistics_pre}
# Plot summary statistics
...
```{r protein_summary_statistics_post}
