...
num_datasets <- length(file_name)
data_dt <- data.table(Data = data.table())
...
# Fro each dataset filter by genes present in all and write to file
for(f in names(data_lst)){
  ...
  fwrite(data_lst[[f]], file = file_write, row.names = T)
}
