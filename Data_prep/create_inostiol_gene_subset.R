...
gene_non_unique_names <- unique(universe[my_universe %in% final_genes_used])

# Change the format to be more in-keeping with previous subsets
inostiol_net <- data.frame(external_gene_name = gene_non_unique_names)

# Save the data in RData format
save(inostiol_net, file = "./Data/250_inostiol_gene_set.RData")
